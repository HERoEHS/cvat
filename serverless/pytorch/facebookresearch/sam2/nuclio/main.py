...
def handler(context, event):
    try:
        context.logger.info("✅ call handler 시작")
        data = event.body
        context.logger.info("📦 request body 가져옴")

        buf = io.BytesIO(base64.b64decode(data["image"]))
        image = Image.open(buf)
        image = image.convert("RGB")
        context.logger.info(f"🖼️ 이미지 변환 완료. size: {image.size}")

        high_res_feats_0, high_res_feats_1, image_embed, image_ = context.user_data.model.handle(image)
        context.logger.info("📊 모델 인퍼런스 완료")
        ...
        context.logger.info("✅ 응답 반환 성공")
        return response

    except Exception as e:
        context.logger.error(f"❌ 에러 발생: {str(e)}", exc_info=True)
